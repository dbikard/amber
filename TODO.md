...
- [x] **A GARRISON STOPS COMMUTING THROUGH ITS OWN GATES — 4,222 transits to 173, and none
      ...
      **What is left, and it is a different animal:** at 240 men against 80 berths the reshuffle
      ...
      the nav layer.
- [x] **A CAMPAIGN: chapters, briefings and objectives — SHIPPED.** Six chapters in
      `js/campaign.js`, each a pinned board, a named rival, a briefing and an objective, with
      predicate-driven hints and a progress record under `amber_campaign`. The sim grew NO third
      win condition: `World.declare` is the one door out, guarded and emitting the same event
      every other ending emits, and the objective is polled from game.js over the world it
      already holds. Objectives so far: `raise`, `hold` (continuously — the clock restarts when
      the count slips), `raze`, `survive`, `walk`, `seat`, plus a `FAIL.lose`. Adding a chapter
      is a table entry; the screen, the lock, the briefing and the end screen follow from it.
      Still to do if it is wanted: a hand-made `spec` for a chapter that needs particular ground
      (`WorldGen.fromSpec` is already validated and `startSP` already takes `opts.spec` — no
